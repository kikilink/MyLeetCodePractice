 ...
 * 向右移动。 由于移动方向相同的行星不会发生碰撞，所以最终没有行星发生碰撞。 说明:
 * 
...
public class AsteroidCollision {
		...
		public int[] asteroidCollision(int[] asteroids) {
			...
			for (int asteroid : asteroids) {
				if (deque.isEmpty()) {
					...
					// 往左才可能相撞
					if (asteroid < 0) {
						int newAsteroid = asteroid;
						while (!deque.isEmpty() && deque.getLast() > 0
								&& newAsteroid < 0) {
							int lastAsteroid = deque.removeLast();
							newAsteroid = (lastAsteroid + newAsteroid) >= 0
									? (lastAsteroid + newAsteroid == 0 ? 0
											: lastAsteroid)
									: newAsteroid;
						}
						if (newAsteroid != 0) {
							deque.addLast(newAsteroid);
						}
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(
				new Solution().asteroidCollision(new int[] { 10, 2, -5 })));

		System.out.println(Arrays.toString(
				new Solution().asteroidCollision(new int[] { 8, -3 })));

		System.out.println(Arrays
				.toString(new Solution().asteroidCollision(new int[] { 1 })));
	}
}
